    ...
    if (!gameOver)
    {
        ...
        for (int j = 0; j < GRID_VERTICAL_SIZE; j++)
        {
            for (int i = 0; i < GRID_HORIZONTAL_SIZE; i++)
            {
                ...
                if (grid[i][j] == EMPTY)
                {
                    DrawLine(offset.x, offset.y, offset.x + SQUARE_SIZE, offset.y, lightGrayColor);
                    DrawLine(offset.x, offset.y, offset.x, offset.y + SQUARE_SIZE, lightGrayColor);
                    DrawLine(offset.x + SQUARE_SIZE, offset.y, offset.x + SQUARE_SIZE, offset.y + SQUARE_SIZE, lightGrayColor);
                    DrawLine(offset.x, offset.y + SQUARE_SIZE, offset.x + SQUARE_SIZE, offset.y + SQUARE_SIZE, lightGrayColor);
                    offset.x += SQUARE_SIZE;
                }
                else if (grid[i][j] == FULL)
                {
                    DrawRectangle(offset.x, offset.y, SQUARE_SIZE, SQUARE_SIZE, grayColor);
                    offset.x += SQUARE_SIZE;
                }
                else if (grid[i][j] == MOVING)
                {
                    DrawRectangle(offset.x, offset.y, SQUARE_SIZE, SQUARE_SIZE, darkGrayColor);
                    offset.x += SQUARE_SIZE;
                }
                else if (grid[i][j] == BLOCK)
                {
                    DrawRectangle(offset.x, offset.y, SQUARE_SIZE, SQUARE_SIZE, lightGrayColor);
                    offset.x += SQUARE_SIZE;
                }
        ...
        DrawText("INCOMING:", offset.x, offset.y - 100, 10, grayColor);
        DrawText(TextFormat("LINES:      %04i", lines), offset.x, offset.y + 20, 10, grayColor);

        if (pause) DrawText("GAME PAUSED", screenWidth / 2 - MeasureText("GAME PAUSED", 40) / 2, screenHeight / 2 - 40, 40, grayColor);
    }
    else DrawText("PRESS [ENTER] TO PLAY AGAIN", GetScreenWidth() / 2 - MeasureText("PRESS [ENTER] TO PLAY AGAIN", 20) / 2, GetScreenHeight() / 2 - 50, 20, grayColor);
...
void DrawVelocity(flecs::world &ecs) {
    ecs.each<Position>([](flecs::entity e, Position const& pos) {
        DrawCircle((int)pos.x, (int)pos.y, 10, BLUE);
    });
}